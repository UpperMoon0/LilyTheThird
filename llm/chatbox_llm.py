...
class ChatBoxLLM(BaseLLMOrchestrator):
    ...
    async def get_response(self, user_message: str) -> tuple[str, None]:
        """
        ...
        Returns:
            A tuple containing the final response string and None (as expected by chat_tab.py).
        """
        print(f"--- Processing ChatBox message ---")
        final_response = await self._process_message(user_message)
        # Return format expected by chat_tab.py
